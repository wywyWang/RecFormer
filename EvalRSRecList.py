...
class EvalRSRecList(RecList):
    ...
    @rec_test('MRED_MOST_DOH')
    def mred_most_date_of_hour(self):
        user_hour = self._x_train[self._x_train['user_id'].isin(self._y_test.index)]
        user_hour["hour"] = pd.to_datetime(user_hour["timestamp"], unit="s").dt.hour
        user_hour = user_hour.groupby('user_id', sort=False)[['hour']].agg(list)
        user_hour['most_hour'] = user_hour['hour'].apply(lambda x: max(set(x), key=x.count))
        user_hour = user_hour.loc[self._y_test.index]

        return self.miss_rate_equality_difference(self._y_preds, self._y_test, user_hour, 'most_hour')
